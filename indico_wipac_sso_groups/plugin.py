...
class WIPACSSOGroupsPlugin(IndicoPlugin):
    """SSO Group Mapping
    ...
    def _user_logged_in(self, user, identity, admin_impersonation, **kwargs):
        ...
        if not identity_provider:
            self.logger.warning('Identity provider not set, plugin ignored')
            return
        group = self.settings.get('sso_group')
        if not group:
            self.logger.warning('Local Users Group not set, plugin ignored')
            return
